    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-4 h-[calc(100vh-64px)] flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3 flex-1 min-h-0">
          ...
          <Card className="bg-white shadow-sm border-0 flex flex-col min-h-0">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-[#5A9BD4] text-base">
                ...
                <span className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded-md">
                  {newTickets ? `${newTickets.length} tickets` : '0 tickets'}
                </span>
              </div>
            </CardHeader>
            <CardContent className="flex-1 overflow-auto px-4 pb-3">
              <div className="space-y-2">
                {(newTickets ?? []).map((ticket) => (
                  <div key={ticket.id} className="border-l-4 border-[#5A9BD4] bg-[#5A9BD4]/5 p-2 rounded-r-lg">
                    ...
                    <h4 className="font-medium text-gray-900 mb-1 text-sm truncate">{ticket.titulo}</h4>
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-700 font-medium truncate">{ticket.solicitante}</span>
                ...
                {(!newTickets || newTickets.length === 0) && (
                  <div className="text-center text-xs text-gray-500 py-4">Nenhum ticket novo</div>
                )}
              </div>
            </CardContent>
          </Card>
